...
export function LibraryRootOverlapWarning({
  ...
  return (
    ...
    >
      ...
      <p>
        GMM blocks Library-wide operations that resolve an overlapping configured root,
        including audit, import, and relocation. Enable, disable, and Junction
        reconciliation still use each Mod&apos;s recorded path, so those operations remain
        available during repair.
      </p>
      <p>
        When you change an unsafe root, GMM updates the setting without reading or
        moving anything from the overlapping path. Choose a different root outside the
        backup tree that does not contain it. Existing Mod records and folders stay
        untouched for manual recovery, and this warning remains until no Mod record
        points there.
      </p>
      {overlaps.length > 0 ? (
        <>
